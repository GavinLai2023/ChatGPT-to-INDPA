...
def read_and_prepare_chatgpt_system_message(system_message_file):
    ...
    system_message_rules = "\n Rules for the generated titles :\n 1.The first row you generate should be a very short, concise, and professional title for this article."\
                    "\n 2. Start with the actual title right away, NO NEED to put stuff like 'Title:' in front."\
                    "\n 3. Conciseness: Give the reader a clear idea of what the article is about without being overly long or complex. The generated titles should ideally less than 6 words, with a strict requirement to be under 8 words."\
                    "\n 4. Capitalisation: Capitalise the first letter of each major word. Articles (a, an, the), coordinating conjunctions (and, but, or), and prepositions (in, on, at, to, etc.) are typically not capitalised unless they are the first or last word of the title. "\
                    "\n 5. Avoiding Complete Sentences: Titles should not be complete sentences. Do not use punctuation marks, like colon mark."\
                    "\n 6. Three title examples: Common Pitfalls in Health and Safety Consultative Processes; Busting Myths in Strategic Planning; Challenges of Strategy Development"
...
def prepare_post_data(ai_response,author,category,tag,WordPress):
    ...
    if title.lower().startswith("Title:".lower()): # If chatGPT generate a "title" prefix, delete it.
...
def main(input_file_name,system_message_file):
    wp = WordPress(wp_username, wp_password, "https://industryprofessor.org/wp-json/wp/v2")
